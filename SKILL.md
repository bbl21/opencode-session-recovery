...
description: "Recover lost opencode conversations after folder rename/move. Auto-relinks stale session.directory pointers via project_id (git root commit hash). Supports --check, --relink, and manual --old/--new remap. Verified on Windows; designed for macOS/Linux."
---
...
But `session.project_id` references `project.id` — a **git root commit hash** (`git rev-list --max-parents=0 --all`) that **stays the same across renames**. This skill uses that stable relationship to relink sessions to the new path, no guessing required.
...
`project_id` is derived from the **git root commit hash**, not the HEAD commit. It is stable during normal development but changes in these scenarios:
